...
class UserManager(BaseUserManager):
  ...
  def create_admin(self, email, password, company, **extra_fields):
    extra_fields.setdefault('is_staff', True)
    extra_fields.setdefault('is_admin', True)
    extra_fields.setdefault('is_active', True)
  ...
  def create_staff(self, email, password, company, **extra_fields):
    extra_fields.setdefault('is_staff', True)
    extra_fields.setdefault('is_active', True)
    user = self.create_user(email, password, **extra_fields)
    Staff.objects.create(user=user, company=company)
...
class Shift(models.Model):
    ...
    staff = models.ManyToManyField(User,related_name='shift_staff')
...
class Conversation(models.Model):
  created_by = models.ForeignKey(User, on_delete=models.SET_NULL, blank=True, null=True, related_name='conversation_creator')
  created_at = models.DateTimeField(auto_now_add=True)
  def __str__(self):
...
class ConversationParticipant(models.Model):
  ...
  user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='conversation_participant')
